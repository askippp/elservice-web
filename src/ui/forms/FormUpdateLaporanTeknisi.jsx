...
import NotifUpdateLaporanTeknisi from '../notif/notifUpdateLaporanTeknisi';

export default function FormUpdateLaporanTeknisi({ teknisi, onClose, onUpdate }) {
  const [formData, setFormData] = useState({
    name: teknisi?.name || '',
    cabang: teknisi?.cabang || '',
    phone: teknisi?.phone || '',
    email: teknisi?.email || '',
    specialization: teknisi?.specialization || '',
  ...
  return (
    <>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8">
        {/* Header */}
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 text-center">Edit Teknisi</h2>
        </div>
      ...
      {showNotif && (
        <NotifUpdateLaporanTeknisi onClose={handleCloseNotif} />
      )}
    </>
  );
}